...
function replaceOrThrow(search, replace, stepName) {
  if (typeof search === 'string') {
    if (!content.includes(search)) throw new Error("Failed step: " + stepName);
  } else {
    if (!content.match(search)) throw new Error("Failed step: " + stepName);
  }
  content = content.replace(search, replace);
  console.log("Success: " + stepName);
}

// 1. Add arrears to ReportTab
replaceOrThrow(
...
// 7. Add filter dropdown options below incomeRows
replaceOrThrow(
  /const incomeRows = useMemo\([\s\S]*?\[invoices, meterByRoomMonth\]\s*\);/,
  function (match) {
...
// 8. Update filteredIncomeRows (also adds date sort)
replaceOrThrow(
...
// 9. Sort moveInRows by date
replaceOrThrow(
  /const moveInRows = useMemo\([\s\S]*?return \[\.\.\.direct, \.\.\.history\]\.sort\(byBuildingAndRoom\);\s*\}, \[logs, roomNumberById, selectedMonth, tenants\]\);/,
  function (match) {
    return match.replace(/return \[\.\.\.direct, \.\.\.history\]\.sort\(byBuildingAndRoom\);/, `return [...direct, ...history].sort((a, b) => {
...
// 10. Sort moveOutRows by date
replaceOrThrow(
  /const moveOutRows = useMemo\([\s\S]*?return \[\.\.\.direct, \.\.\.history\]\.sort\(byBuildingAndRoom\);\s*\}, \[logs, roomNumberById, selectedMonth, tenants\]\);/,
  function (match) {
    return match.replace(/return \[\.\.\.direct, \.\.\.history\]\.sort\(byBuildingAndRoom\);/, `return [...direct, ...history].sort((a, b) => {
      const dateA = new Date(a.date || 0).getTime();
      const dateB = new Date(b.date || 0).getTime();
...
// 11. Update filteredMovementRows
replaceOrThrow(
...
console.log("Fully rebuilt ReportsPageView!");
