...
// 1. Add state variables at the top of ReportsPageView
const stateSearch = `  const [activeTab, setActiveTab] = useState<ReportTab>("income");
  const [loading, setLoading] = useState(true);`;
const stateReplace = `  const [activeTab, setActiveTab] = useState<ReportTab>("income");
  const [loading, setLoading] = useState(true);
...
// 2. Add Modal import if missing, wait Modal is in @/components/ui/Modal
// Let's add Search import
const importSearch = `import { Download, FileSpreadsheet, Loader2 } from "lucide-react";`;
const importReplace = `import { Download, FileSpreadsheet, Loader2, Search } from "lucide-react";\nimport { Modal } from "@/components/ui/Modal";`;
if (content.includes(importSearch)) {
    content = content.replace(importSearch, importReplace);
}

// 3. Update filteredIncomeRows
const filteredIncomeSearch = `  const filteredIncomeRows = useMemo(
    () => incomeRows.filter((row) => row.month === selectedMonth).sort(byBuildingAndRoom),
    [incomeRows, selectedMonth]
  );`;
const filteredIncomeReplace = `  const filteredIncomeRows = useMemo(() => {
...
if (content.includes(filteredIncomeSearch)) {
    content = content.replace(filteredIncomeSearch, filteredIncomeReplace);
}

// 4. Update movementRows (filteredMovementRows)
// Find `const movementSummary = useMemo(`
const movementSearch = `  const movementSummary = useMemo(
    () => ({
...
const movementReplace = `  const movementSummary = useMemo(
    () => ({
      total: movementRows.length,
      moveIn: movementRows.filter((row) => row.type === "ย้ายเข้า").length,
      moveOut: movementRows.filter((row) => row.type === "ย้ายออก").length,
...
if (content.includes(movementSearch)) {
    content = content.replace(movementSearch, movementReplace);
}

// 5. Replace Income tab UI to add search and custom table
const incomeTabSearch = `              <div className="flex flex-wrap items-end gap-3">
...
const incomeTabReplace = `              <div className="flex flex-wrap items-end gap-3">
                  ...
                  <tbody className="border border-slate-100">
                    ...
                    {Array.isArray(selectedIncomeInvoice.additional_fees_breakdown) && selectedIncomeInvoice.additional_fees_breakdown.map((fee: any, idx: number) => (
                      <tr key={idx} className="border-b border-slate-100">
                        ...
                        <td className="px-3 py-2 text-right">{formatMoney(toNumber(fee.amount || fee.total_amount))}</td>
                      </tr>
                    ))}
...
if (content.includes(incomeTabSearch)) {
    content = content.replace(incomeTabSearch, incomeTabReplace);
}

// 6. Replace Movement tab UI
const movementTabSearch = `              <div className="flex items-center justify-between gap-3">
                <div>
...
const movementTabReplace = `              <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-slate-900">รายงานการเคลื่อนไหวห้องและผู้เช่า</h3>
                  <p className="text-sm text-slate-500">ดูว่าห้องไหนมีการย้ายเข้า ย้ายออก หรือย้ายห้องเมื่อใด</p>
                </div>
...
if (content.includes(movementTabSearch)) {
    content = content.replace(movementTabSearch, movementTabReplace);
}

fs.writeFileSync(path, content);
console.log("Done updating ReportsPageView");
