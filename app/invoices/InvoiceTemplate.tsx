...
export default function InvoiceTemplate({ data, settings, onClose }: any) {
  ...
  return (
    <div 
      ...
      className="fixed inset-0 z-[100] bg-gray-900/95 flex flex-col items-center overflow-auto p-4 md:p-8"
    >
      {/* TOOLBAR (Strictly Hidden in Print) */}
      <div className="no-print w-full max-w-[210mm] flex justify-between items-center mb-6 text-white shrink-0">
        ...
        <div className="flex gap-3">
          <button onClick={handlePrint} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-full font-bold shadow-lg flex items-center gap-2 transition-transform hover:scale-105">
            🖨️ Print / Save as PDF
          </button>
          <button onClick={onClose} className="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-full font-bold">
            Close X
          </button>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex justify-between items-start border-b-2 border-gray-800 pb-6 mb-8">
          ...
          <div className="w-2/3">
            ...
            <p className="text-sm text-gray-600 whitespace-pre-line">{settings?.address || 'Address not set'}</p>
          ...
          <div className="w-1/3 text-right">
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-mono font-bold">{invoice.id.substring(0,8).toUpperCase()}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-bold text-gray-500">Room:</span>
                <span className="font-bold text-lg">{invoice.rooms?.room_number}</span>
              </div>
              <div className="flex justify-between">
          ...
          <tbody className="text-sm">
            ...
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 text-right text-gray-500">
                {invoice.electric_units} Units x {invoice.unit_price_elec}
              </td>
            ...
            <tr className="border-b border-gray-100">
              ...
              <td className="py-3 text-right text-gray-500">
                {invoice.water_units} Units x {invoice.unit_price_water}
              </td>
          ...
          <div className={`text-right border-t-2 ${borderClass} pt-2 w-1/2`}>
            <div className="flex justify-between items-end">
              ...
              <span className={`text-4xl font-black ${colorClass}`}>
                {Number(invoice.total_amount).toLocaleString()} <span className="text-lg">THB</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-12 mt-auto pt-12 border-t border-gray-200">
          <div>
            <h3 className="font-bold text-sm uppercase text-gray-400 mb-2">
              {isReceipt ? 'Note' : 'Payment Details'}
            </h3>
            {isReceipt ? (
               <p className="text-sm text-gray-600">This document serves as an official receipt for the payment of the above period.</p>
            ) : (
